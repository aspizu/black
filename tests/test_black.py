...
class BlackTestCase(BlackBaseTestCase):
   ...
   def test_one_empty_line_ff(self) -> None:
      ...
      for nl in ["\n", "\r\n"]:
         ...
         try:
            self.assertFalse(ff(tmp_file, mode=mode, write_back=black.WriteBack.YES))
   ...
   def test_get_features_used(self) -> None:
      ...
      self.assertEqual(black.get_features_used(node), {Feature.ANN_ASSIGN_EXTENDED_RHS})
   ...
   def test_normalize_path_ignore_windows_junctions_outside_of_root(self) -> None:
      ...
      with TemporaryDirectory() as workspace:
         ...
         report = black.Report(verbose=True)
         normalized_path = black.normalize_path_maybe_ignore(junction_dir, root, report)
